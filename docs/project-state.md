...
> 这是**可更新的操作状态**，不是第二套投资思想、实时行情数据库或已批准投资结论。最后人工核对：2026-09-18（北京时间）；该日期以后的账户状态、市场行情及自动任务运行情况须重新查询，不能根据本页推断。发生重要变化后由执行者更新本页并给出可验证回执。
...
- 投资思想库 [`../philosophy/core-beliefs.md`](../philosophy/core-beliefs.md) 当前是 **v0.3 意旨提炼草案**；用户的明确执行要求可执行，但不能宣称整份原则已被逐字审定。研究中的事实／推论／情景／未核实信息保持区分，AI 建议或可能的预期差不等于所有者批准或市场证实。
...
- 参考外部 HARC 后建议的更严格人类批准门、强制中英双语维护、项目许可选择：**未由所有者授权采纳**；本轮只做兼容现有流程的轻量交接和明确的来源/状态区分，详见 [`collaboration-review.md`](collaboration-review.md)。
