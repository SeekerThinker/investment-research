...
LIBRARY_FILES = (
    ...
    "idea-20260919-01.md", "integration-map.md", "change-log.md",
)
METHODS = (
...
def main() -> None:
    ...
    ai_idea = (LIBRARY / "idea-20260919-01.md").read_text(encoding="utf-8")
    ...
    require(len(principles) >= 11, "P01–P11 must remain present")
    ...
    require(len(idea_ids) >= 12, "all historical IDEA rows and the AI thesis must remain")
    ...
    require("IDEA-20260919-01" in idea_ids and "idea-20260919-01.md" in ideas,
            "AI thesis or original intent record missing from register")
    require("**P11｜" in beliefs and "所有者" in ai_idea and "编辑者" in ai_idea,
            "AI thesis owner/editor provenance missing")
    for name, text in (("philosophy/change-log.md", changelog),
                       ("philosophy/integration-map.md", mapping),
                       ("metadata/philosophy-integration.md", integration)):
        require("IDEA-20260919-01" in text and "P11" in text,
                f"{name} has not propagated AI long-term thesis")
    for marker in ("需求", "支出", "现金流", "反证"):
        require(marker in integration, f"AI study lacks conditional verification: {marker}")
